...
export const ScenarioAnalysis = ({ 
  ...
  const [localFeatureChanges, setLocalFeatureChanges] = useState(scenarioFeatureChanges);
  const [localConfig, setLocalConfig] = useState(scenarioConfig);

  // Sync with parent when it changes (e.g., reset)
  // Since parent only updates on commit, this won't interrupt dragging
  useEffect(() => {
    setLocalFeatureChanges(scenarioFeatureChanges);
  }, [scenarioFeatureChanges]);
  ...
  const getTopNClusters = (clustersArr: any[], n: number, sortBy: 'score' | 'number' = 'number') => {
    if (!Array.isArray(clustersArr)) return [];
    return [...clustersArr]
      .sort((a, b) => {
        if (sortBy === 'score') {
          return (b.avg_suitability_score || 0) - (a.avg_suitability_score || 0);
        }
        return (a.cluster_number || 0) - (b.cluster_number || 0);
      })
      .slice(0, n);
  };
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      {((scenarioFeatureChanges[feature] || 0) * 100).toFixed(1)}%
                        ...
                        <Slider
                    value={[localFeatureChanges[feature] || 0]}
                    onValueChange={([value]) => setLocalFeatureChanges(prev => ({...prev, [feature]: value}))}
                    onValueCommit={([value]) => onFeatureChange(feature, value)}
                    min={-0.5}
                    max={0.5}
                    step={0.01}
              className="w-full"
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {localConfig.villagePercentage.toFixed(1)}%
                  </Badge>
            </div>
            <Slider
                  value={[localConfig.villagePercentage]}
                  onValueChange={([value]) => setLocalConfig(prev => ({...prev, villagePercentage: value}))}
                  onValueCommit={([value]) => onConfigChange({ villagePercentage: value })}
              min={0}
                  max={100}
                  step={1}
              className="w-full"
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {localConfig.randomnessFactor.toFixed(1)}%
                  </Badge>
                </div>
                <Slider
                  value={[localConfig.randomnessFactor]}
                  onValueChange={([value]) => setLocalConfig(prev => ({...prev, randomnessFactor: value}))}
                  onValueCommit={([value]) => onConfigChange({ randomnessFactor: value })}
                  min={0}
                  max={20}
                  step={0.5}
                  className="w-full"
      ...
      {scenarioClusters.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {getTopNClusters(scenarioClusters, 20, 'number').map((cluster, index) => (
                <ClusterCard
      ...
      {isScenarioScored && scenarioClusters.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-muted-foreground">
              Please run baseline clustering first to see scenario impact on specific clusters.
            </p>
          </CardContent>
        </Card>
      )}
